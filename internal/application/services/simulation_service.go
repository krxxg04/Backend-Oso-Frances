...
func ValidateSimulationInput(in domain.SimulacionInput) []domain.APIError {
	...
	if shared.NormalizeKey(in.NombreCliente) == "" {
...
func CalculateSimulation(in domain.SimulacionInput) domain.SimulacionResult {
	...
	resumen := domain.FinancialSummary{
		...
		TotalPagado:       shared.Round2(costoTotal),
		VAN:               van,
		TIR:               tir,
		TCEA:              tcea,
	...
	return domain.SimulacionResult{
		...
		TIR:             tir,
		TCEA:            tcea,
		CostoTotal:      shared.Round2(costoTotal),
		TotalIntereses:  shared.Round2(totalIntereses),
		TotalSeguros:    shared.Round2(totalSeguros),
		CuotaInicial:    shared.Round2(cuotaInicial),
		MontoNeto:       shared.Round2(montoNeto),
		MontoFinanciado: shared.Round2(montoFinanciado),
		Resumen:         resumen,
		Cronograma:      cron,
	}
...
func buildPago(mes int, fechaInicio time.Time, hasFecha bool, saldoInicial, seguro, seguroVehicular, seguroDesgravamen, cuotaCapitalInteres, interes, amortizacion, saldoFinal float64, tipoGracia domain.GraceType) domain.Pago {
	...
	return domain.Pago{
		...
		Fecha:               fecha,
		FechaPago:           fechaPago,
		SaldoInicial:        shared.Round2(saldoInicial),
		Cuota:               shared.Round2(cuotaTotal),
		CuotaCapitalInteres: shared.Round2(cuotaCapitalInteres),
		Interes:             shared.Round2(interes),
		SeguroVehicular:     shared.Round2(seguroVehicular),
		SeguroDesgravamen:   shared.Round2(seguroDesgravamen),
		Seguro:              shared.Round2(seguro),
...
func matchesSimulationFilter(sim domain.Simulacion, filter domain.SimulacionFilter) bool {
	...
	if filter.Vehiculo != "" {
		needle := shared.NormalizeKey(filter.Vehiculo)
		haystack := shared.NormalizeKey(strings.TrimSpace(sim.Input.Vehiculo.Marca + " " + sim.Input.Vehiculo.Modelo + " " + sim.Input.Vehiculo.Tipo))
