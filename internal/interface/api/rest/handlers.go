...
func NewHandler(cfg config.Config, authSvc *services.AuthService, simSvc *services.SimulationService, vehSvc *services.VehicleService, limiter *LoginLimiter) *Handler {
...
func (h *Handler) CreateSimulation(c *gin.Context) {
	...
	in, errs := services.ApplySimulationRules(in)
	if len(errs) > 0 {
...
func (h *Handler) CreateVehicle(c *gin.Context) {
	...
	if errs := services.ValidateVehicle(in); len(errs) > 0 {
		c.JSON(http.StatusBadRequest, domain.ErrorResponse{Error: domain.NewError("validation_error", "errores de validacion", ""), Errors: errs})
		return
	}
	usernameAny, _ := c.Get("username")
...
func (h *Handler) ListBankOptions(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"items": services.BankOptions()})
}
